...
def mfa_search(last_date):
    """外交部发言人答记者问/例行记者会页直抓
    9/3修正: fyrbt 栏目本质是表态渠道(例行记者会/答记者问)，不作为事件候选源。
    仅当标题含实质性动作宣布(反制/列入/实施/暂停/启动)才保留；纯表态全部剔除。"""
    ...
    for m in re.finditer(r'<a href="\./(20260?\d/t\d+_\d+\.shtml)"[^>]*>(2026年[^<]{5,100})</a>', html):
        ...
        title = m.group(2).strip()
        title = re.sub(r'\s+', ' ', title)
        # 例行记者会/答记者问 = 纯表态渠道，非实质动作 → 剔除（9/3事故修正）
        if any(k in title for k in ['例行记者会', '答记者问', '答问', '记者会']):
            continue
        # 从标题提取日期 2026年8月17日
...
12. **"回顾旧动作"识别（硬性）★★**：候选若正文明确引用 last_date 之前的动作日期（如"8月13日公告""8月24日制裁""8月26日行政令"）或既有命令编号（第14421号/FR Doc 2026-xxx），且其"新意"仅是解读、预测后续影响——**不收为新事件**，判为旧事件解读。只有官方正式发布的新动作（新FR Doc/新名单/新命令/新公告）才收。
...
14. **中方"表态类"不收录（硬性·9/3事故新增）★★★**：中方**答记者问、例行记者会、发言人表态、使馆回应**等口头表态**不作为独立事件收录**——它们不是实质反制手段，只是对已发生动作（多为已收录的美方动作）的回应。判据：行动字段若仅含"表示不满/坚决反对/将采取必要措施/回应关切"等措辞、无任何**新**实质性措施（新列清单/新出口管制/新调查/新暂停进口），**一律不收**。时间线中方侧只收实质动作本身（"将X列入不可靠实体清单""对X启动反倾销调查"），且该动作已在公告/命令/调查通知正式落地，而非仅在记者会上宣布意向。9/3 事故：商务部答记者问3类+外交部例行记者会1类共6条表态被误收，全部删除。
15. **商务部/外交部直抓条目筛选**：商务部 xwfb/zcfb 与外交部 fyrbt 列表页大量是**答记者问/例行记者会**条目，默认**不收为事件**；仅当标题/正文表明发布了**新的实质动作**（新公告/新令/新清单/新调查/新暂停进口/新管制）才收该动作。外交部例行记者会涉美内容复杂，纯回应表态一律不收，仅在宣布新的实质反制措施时收录该措施。
"""
...
def main():
    ...
    DUP_KEYS = ['长鑫', 'CXMT', '英伟达', 'Nvidia', '白名单', '无人机', '芯片关税', 'laptop', 'server', '1260H', '实体清单',
                'SDN', 'Kameng', '电力设备', 'BESS', '储能', '电网', '行政令', 'Executive Order', '14421',
